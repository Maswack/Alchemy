...
execute as @s at @s run summon item ~ ~ ~ {Item:{id:"minecraft:chicken_spawn_egg",Count:1b,tag:{display:{Name:'{"text":"Poskładana Stacja Alchemiczna","color":"dark_aqua","bold":true}'},EntityTag:{Silent:1b,DeathLootTable:"minecraft:empty",NoAI:1b,Tags:["AC_stationReplacer"]}}}}

execute at @s run playsound minecraft:block.respawn_anchor.deplete master @e[type=player,distance=0..10] ~ ~ ~ 2.0 1.5
execute at @s run particle minecraft:nautilus ~ ~1 ~ 0.15 0.15 0.15 0.15 250
